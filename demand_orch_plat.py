import numpy as np
def forecast_demand_orch_plat(history, horizon=12, alpha=0.3, beta=0.15):
    n = len(history)
...
if __name__=="__main__":
    np.random.seed(42)
    hist = (np.cumsum(np.random.normal(4, 10, 52)) + 3000).tolist()
    r = forecast_demand_orch_plat(hist)
